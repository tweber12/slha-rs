...
use {Decay, Line, Parseable, ParseableWord, RawBlock, SlhaBlock};
use errors::*;
...
fn parse_block_header(header: &str) -> Result<(String, Option<f64>)> {
    ...
    let scale = parse_block_scale(rest).chain_err(|| ErrorKind::InvalidBlock(name.clone()))?;
    Ok((name, scale))
}
